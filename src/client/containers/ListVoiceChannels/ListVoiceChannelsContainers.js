...
class ListVoiceChannelContainers extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return !(
      nextProps.location.path === this.props.location.path &&
      nextState === this.state
    );
  }

  componentDidMount() {
    this.props.infoFromDiscord
      ? this.setState({ listOfBannedWords: ["TEST", "TEST1"] })
      : this.setState({ listOfBannedWords: ["TEST", "TEST1"] });
  }
    ...
    if (infoFromDiscord) {
      return (
        <div className="content-server">
          <Col className="content-server-col24" span={24}>
            <ListChannelContainer listOfGuilds={infoFromDiscord} />
          </Col>
        </div>
      );
    } else {
